...
class MSVDQA(MSVDQA_Raw):
    def __init__(self, train_batch_size, preprocess_dir, max_frame_num, **kwargs):
        super().__init__(train_batch_size, preprocess_dir)
        self.max_frame_num = max_frame_num
        self.vocab = torch.from_numpy(np.load(preprocess_dir + "/word_embedding.npy"))

    def transform_data(self, vgg_batch, c3d_batch, question_batch, answer_batch, question_lengths, example_id=None):
        vgg_feat = torch.tensor(vgg_batch).float()
        c3d_feat = torch.tensor(c3d_batch).float()
        question_idx = torch.tensor(question_batch).long()
        if not isinstance(answer_batch, str):
            answer = torch.tensor(answer_batch).long()
        else:
            answer = answer_batch
        raw_visual_feat = torch.cat((vgg_feat, c3d_feat), dim=-1)
        visual_feat = torch.zeros(raw_visual_feat.size(0), self.max_frame_num, raw_visual_feat.size(-1))
        frame_idx = 1.0 * torch.arange(0, self.max_frame_num + 1) / self.max_frame_num * raw_visual_feat.size(1)
        frame_idx = torch.round(frame_idx).long()
        frame_idx[frame_idx >= visual_feat.size(1)] = visual_feat.size(1) - 1
        for j in range(self.max_frame_num):
            s, e = frame_idx[j], frame_idx[j + 1]
            assert s <= e
            if s == e:
                visual_feat[:, j] = visual_feat[:, s]
            else:
                visual_feat[:, j] = visual_feat[:, s:e].mean(dim=1)

        question_embed = F.embedding(question_idx, self.vocab)
        question_lengths = torch.tensor(question_lengths)
        question_mask = sequence_mask(question_lengths, question_embed.size(1))
        return {
            "net_input": {
                "textual_feat": question_embed,
                "textual_mask": question_mask,
    ...
    def get_train_batch(self):
        return self.transform_data(*(super().get_train_batch()))

    def get_val_example(self):
        return self.transform_data(*(super().get_val_example()))

    def get_test_example(self):
        return self.transform_data(*(super().get_test_example()))
