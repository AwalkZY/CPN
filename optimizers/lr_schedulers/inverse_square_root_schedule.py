...
class InverseSquareRootSchedule(FairseqLRScheduler):
    ...
    We also support data warmup phase where we linearly increase the learning rate
    from some initial learning rate (``--warmup-init-lr``) until the configured
    learning rate (``--lr``). Thereafter we decay proportional to the number of
    updates, with data decay factor set to align with the configured learning rate.
    ...
    def __init__(self, args, optimizer):
        ...
        # if len(args.lr) > 1:
        #     raise ValueError(
        #         'Cannot use data fixed learning rate schedule with inverse_sqrt.'
